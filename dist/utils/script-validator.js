 ...
 */
export function validateCustomNodeScript(scriptCode, outputs, inputs) {
    const warnings = [];
    ...
    if (!hasProcessData) {
        warnings.push({
            ...
            severity: 'error',
            message: "No 'processData' function found. Script must define: function processData(inputs, properties) { return { ... }; }",
            line: 1,
        });
    }
    ...
    if (processDataNode) {
        ...
        if (params.length === 0) {
            warnings.push({
                type: 'best_practice',
                severity: 'warning',
                message: 'processData has no parameters. Expected: function processData(inputs, properties)',
            ...
            warnings.push({
                type: 'best_practice',
                severity: 'info',
                message: "processData has 1 parameter. Consider adding 'properties' as second parameter for node configuration.",
                line: getNodeLine(processDataNode),
            });
        }
    }
    ...
    if (resultMatch && !outputs.find((o) => o.name === 'result')) {
        ...
        if (!/return\s*\{[^}]*result[^}]*\}/.test(scriptCode)) {
            ...
            warnings.push({
                type: 'best_practice',
                severity: 'info',
                message: "Variable 'result' defined but may not be returned. Did you mean to include it in the return statement?",
                line: lineNum,
            });
        }
    }
...
export function formatWarnings(warnings) {
    ...
    const lines = ['', '## Script Validation Warnings:', ''];
    // Group by severity
...
function findReturnedKeys(funcNode) {
    const keys = new Set();
    walkNode(funcNode.body, (node) => {
        if (node.type === 'ReturnStatement') {
                ...
                for (const prop of objExpr.properties) {
                    if (prop.type === 'Property') {
                        ...
                        if (propNode.key.type === 'Identifier') {
                            keys.add(propNode.key.name);
                        }
                        else if (propNode.key.type === 'Literal') {
                            const value = propNode.key.value;
