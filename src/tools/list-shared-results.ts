...
function formatSharedResult(result: SharedResult): string {
  const lines = [
...
export async function handleListSharedResults(
  api: FlowDotApiClient,
  args: { workflow_id: string; sort?: string; limit?: number; page?: number }
): Promise<CallToolResult> {
  try {
    const result = await api.listSharedResults(args.workflow_id, {
      sort: args.sort as 'newest' | 'oldest' | 'most_viewed',
      limit: args.limit,
      page: args.page,
    });
