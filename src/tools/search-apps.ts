...
import type { AppSearchFilters } from '../types.js';
...
export async function handleSearchApps(
  ...
  try {
    ...
    if (args.query) filters.q = String(args.query);
    if (args.category) filters.category = String(args.category);
    if (args.tag) filters.tag = String(args.tag);
    if (args.mobile_compatible) filters.mobile_compatible = Boolean(args.mobile_compatible);
    if (args.sort) filters.sort = args.sort as AppSearchFilters['sort'];
    if (args.limit) filters.limit = Number(args.limit);
    if (args.page) filters.page = Number(args.page);
